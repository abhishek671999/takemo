...
export class ForgotpasswordComponent {
  constructor(private _fb: FormBuilder, private _signupService: SignupService, private _pwdRecoveryService: PwdrecoveryService){}
  ...
  authUser(){
    if (this.authNotRequested && this.pwdRecoverForm.controls.username.valid){
      ...
      this._pwdRecoveryService.authUser(this.pwdRecoverForm.value.username).subscribe(
      data => {
  ...
  newPwdSubmit(){
    this._pwdRecoveryService.updatePwd(this.pwdRecoverForm.value.username, this.otpForm.value.otp, this.pwdRecoverForm.value.newPassword).subscribe(
      data => {
        console.log(data);
      },
