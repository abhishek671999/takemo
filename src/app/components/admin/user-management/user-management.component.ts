...
export class UserManagementComponent {
  ...
  showErrorDialog(msg: string) {
    let dialogRef = this.dialog.open(ErrorMsgDialogComponent, {
      data: { msg: msg },
    });
    setTimeout(() => {
      dialogRef.close();
  ...
  deleteRule(rule) {
    ...
    let bodyParams = {"rule_id": rule.id}
    let additionalData = {'rule_name': rule.name}
    let dialogData = {bodyParams: bodyParams, additionalData: additionalData}
    let dialogRef = this.dialog.open(DeleteRuleConfirmationComponent, {
      data: dialogData,
    });
    this.handlePostDialogClosure(
      dialogRef,
  ...
  addRule() {
    ...
    this.handlePostDialogClosure(
      ...
      'Failed to add rule. Please contact Takemo'
    );
  }
}
